...
export function TicketComments({ ticket, tenant, initialComments }) {
  ...
  useEffect(() => {
    // Listener for comment changes. INSERT, DELETE and UPDATE
    ...
    // Listener for attachment changes. INSERT
            ...
            uploadPromise = Promise.all(
              Array.from(fileList).map((file) => 
                supabase.storage
                  ...
                  .upload([tenant, ticket, getRandomHexString(), file.name].join('/'), file)))
          }
            ...
            {comment.comment_attachments?.length > 0 && (
              <>
                ...
                {comment.comment_attachments.map((attachment) => (
                  ...
                  }} key={attachment.id} className="file-badge">
